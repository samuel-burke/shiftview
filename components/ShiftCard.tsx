...
import { Employee, Schedule, ShiftType, getShiftType, isHere, SHIFT_COLORS } from "../data/mockData";
...
export default function ShiftCard({ employee, schedule, nowMinutes, onClick }: Props) {
  ...
  if (scheduled && !here && schedule.startMinutes > nowMinutes) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {shiftType && (
          <div style={{ fontSize: 11, color: shiftColor, marginTop: 2, textTransform: "capitalize" }}>
            {shiftType}
          </div>
        )}
      </div>
      ...
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        ...
        <div style={{ marginTop: 5, display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 6 }}>
          {arrivalText && (
            <span style={{ fontSize: 10, color: "#94a3b8" }}>{arrivalText}</span>
          )}
          {here && (
            <span style={{
