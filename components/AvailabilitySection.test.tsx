...
describe("AvailabilitySection — notes", () => {
  beforeEach(() => {
    vi.stubGlobal("fetch", makeMockFetch([]));
  });

  it("shows '+ Add note' button when Window state is selected", async () => {
    render(<AvailabilitySection {...BASE_PROPS} />);
    await act(async () => { await Promise.resolve(); });
    await openSheet(1); // Monday
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Window" })); });
    await act(async () => { await Promise.resolve(); });
    expect(screen.getByRole("button", { name: "Add note" })).toBeInTheDocument();
  });

  it("shows '+ Add note' button when Off state is selected", async () => {
    render(<AvailabilitySection {...BASE_PROPS} />);
    await act(async () => { await Promise.resolve(); });
    await openSheet(1);
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Off" })); });
  ...
  it("does not show note UI when Any time is selected", async () => {
    render(<AvailabilitySection {...BASE_PROPS} />);
    await act(async () => { await Promise.resolve(); });
    await openSheet(1);
  ...
  it("displays existing note loaded from API as preview", async () => {
    vi.stubGlobal("fetch", makeMockFetch([
      { id: 1, dayOfWeek: 1, startMinutes: 720, endMinutes: 1320, note: "After school drop-off" },
    ]));
    render(<AvailabilitySection {...BASE_PROPS} />);
    await act(async () => { await Promise.resolve(); });
    await openSheet(1);
    // Note appears in the sheet (inside availability-sheet) with Edit/Clear buttons
    expect(screen.getByRole("button", { name: "Edit note" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Clear note" })).toBeInTheDocument();
    expect(screen.getAllByText(/After school drop-off/).length).toBeGreaterThan(0);
  });

  it("shows note preview on collapsed row when note exists", async () => {
    vi.stubGlobal("fetch", makeMockFetch([
      { id: 1, dayOfWeek: 1, startMinutes: null, endMinutes: null, note: "Family time" },
    ]));
    render(<AvailabilitySection {...BASE_PROPS} />);
    await act(async () => { await Promise.resolve(); });
    const row = screen.getByTestId("day-row-1");
    expect(row.textContent).toMatch(/Family time/);
  });
});
